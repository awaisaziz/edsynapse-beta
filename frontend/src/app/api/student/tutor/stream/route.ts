...
import { groundedContext, streamTutorReply, updateTutorMemory, EMPTY_MEMORY, type TutorMemory } from "@/lib/llm"
...
export async function POST(req: NextRequest) {
  ...
  // Storage key for the persisted conversation. Defaults to `topic`, but surfaces
  // like the course-wide Chat tab pass a sentinel key (e.g. "__course_chat__") so
  // their history stays separate WITHOUT leaking that sentinel into the retrieval
  // query or the system prompt (those still use the human-readable `topic`).
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        let full = ""
        for await (const delta of streamTutorReply({
          topic,
          message,
          context,
          history,
        ...
        // Persist the exchange and refresh durable memory. The visible chat
        // (messages) is short-term; memory survives a chat "clear". Memory
        // update runs after `done` so it never delays the streamed reply.
        ...
        console.error("[v0] tutor stream error:", err)
